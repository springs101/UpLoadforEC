    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if IdentifyFileExist(storagefile.excelFile.path):
                return HttpResponse(
                    "文件已经上传完毕！请检查", content_type="application/json")
            try:
                storagefile.save()
...
def filter_excel(workbook, type, column_name=0):
    ...
    for one_row in range(1, total_rows):  # 也就是从Excel第二行开始，第一行表头不算
        ...
        if row:
            ...
            if type == '钻展-单品' or type == '钻展-全店':
                row_object["Conversion_time"] = getchangetime(
                    MinDate, changeXlrdTimeToDate(row[3]))
    ...
    if type == '超级推荐':
        ...
        try:
            models.DrillingExhibitionSingleProducts.objects.bulk_create(
                outlist)
        except Exception as err:
            print(err)
    elif type == '钻展-全店':
...
def compareHead(srource, type):
    ...
    for line in obj:
        if not line.download_column in srource:
            if not line.download_column in [
                    '主键ID', '导入时间', '转化时间', '产品名称', '店铺名称']:
    ...
    if type == '超级推荐':
        for val in excel_list:
            newobj = models.SuperRecommendation(date=val['date'],
                                                ...
                                                number_of_sneakers_guided_into_stores=val[
                                                    'Number_of_sneakers_guided_into_stores'],
                                                ...
                                                average_number_of_pages_accessed=val[
                                                    'Average_number_of_pages_accessed'],
                                                ...
                                                add_the_number_of_shopping_carts=val[
                                                    'Add_the_number_of_shopping_carts'],
        ...
        for val in excel_list:
            newobj = models.ThroughTrain(date=val['date'],
                                         ...
                                         indirect_transaction_amount_points=val[
                                             'Indirect_transaction_amount_points'],
        ...
        for val in excel_list:
            newobj = models.DrillingExhibitionSingleProducts(basic_planning_information=val['Basic_Planning_Information'],
                                                             ...
                                                             basic_information_of_extension_unit=val[
                                                                 'Basic_Information_of_Extension_Unit'],
                                                             time=val['time'],
                                                             to_show=val['To_show'],
                                                             click=val['click'],
                                                             consume=val['consume'],
                                                             ...
                                                             thousands_of_exhibition_costs_yuan=val[
                                                                 'Thousands_of_exhibition_costs_yuan'],
                                                             visitor=val['Visitor'],
                                                             deep_store_entry=val['Deep_Store_Entry'],
                                                             length_of_visit=val['Length_of_visit'],
                                                             number_of_pages_accessed=val['Number_of_pages_accessed'],
                                                             collection_of_baby_quantity=val[
                                                                 'Collection_of_Baby_Quantity'],
